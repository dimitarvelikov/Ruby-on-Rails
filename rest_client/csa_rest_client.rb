...
class CSARestClient
  ...
  def display_menu(option)
    case option
      when '0'
        puts 'Enter option: '
        puts '1. Enter Threads menu'
        puts '2. Enter Users menu'
      when '1'
        puts 'Enter option: '
        puts '1. Display threads'
        puts '2. Create new thread'
        puts 'B. Back'
      when '2'
        puts 'Enter option: '
  ...
  def display_thread
    begin
      response = RestClient.get "#{@@DOMAIN}/api/comments.json?all", authorization_hash
  ...
  def create_thread
    begin
      ...
      # File object.
      response = RestClient.post "#{@@DOMAIN}/api/comments.json",
